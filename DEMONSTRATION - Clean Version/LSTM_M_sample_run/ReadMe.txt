In this zip, there is a csv file named normalized_training which contains training data samples (after normalization). In addition, there is a csv file named normalized_testing which contains testing data samples (after normalization) of active region #12320.
...
The column headers for a data sample in each csv file are 
	...
	- 22 features where the order of the features is consistent with the order of features described in Figure 6.
...
The usage is given as follows:
	python3 LSTMflare.py M 0

The first argument "LSTMflare.py" is the Python program file name.
The second argument "M" denotes that the program will make predictions of >=M class flares.
